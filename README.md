...
Untrusted input is a chat command; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a chat command
- **Closed type:** `type Decision = GameCmd(GCmd) | IgnoreCmd` where `GCmd` is closed
- **Sink / capability:** `grant runGameCmd` → `using runGameCmd { privileged { runGameCmd(d) } }`
- **Consequence axes:** Trust, Capabilities

## Run the demo

```sh
examples/apps/game-command/demo.sh
```
...
- `game-command_safe.eide` — the correct design.
- `game-command_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
